# CSCI 4302/5302 Final - Reverse Drive
# Team Maleficent - Andy Ho, Matt Ward, Mohammed Adib Oumer, Rameez Wajid
# Fall 2023

# run in new terminal:
# cd project_ws && 
# source install/setup.bash && ros2 run robot_drive drive_backward_PID
...
class BackwardDriver(Node):
    def __init__(self):
        # inside the node, create publisher, subscriber
        super().__init__('backward_driver')
        ...
        self.turn_cruise = self.cruise*0.9 # relative speed/throttle as it turns right at a hallway
        ...
        # PID right wall follow - straight through the hallway (1,0, 0.9)
        self.k_p = 0.85 # pushes robot right reading towards the setpoint with this constant
        self.k_i = 0.0 # steady state error not too relevant in our case
        self.k_d = 0.8 # adjusts rate of reaching goal - slow down if setpoint is close
        self.setpoint = 0.8 # maintain a distance of 0.7m from the wall
        ...
        # PID right turn - controls turning when the right side LIDAR readings are huge (2.0, 0.0, 1.7)
        self.k_p_turn = 1.0 # pushes robot right reading towards the setpoint with this constant
        self.k_i_turn = 0.0 # steady state error not too relevant in our case
        self.k_d_turn = 0.9 # adjusts rate of reaching goal
        self.setpoint_turn = 1.0 # distance to maintain as robot turns from the wall
        ...
        self.pid_turn.output_limits = (-0.6, 0.6) # wheel angle adjustments should be a lot more than driving straight
    ...
    def sub_callback(self, msg):
        ...
        back_msg  = received_msg[245:305] #
        min_back_distance = np.min(back_msg)
        left_msg = received_msg[400:500] #370:500 # indices might need adjustment based on LIDAR
        min_left_distance = np.min(left_msg)
        right_msg = received_msg[130:200] #80:210  # not being used now, can use it if driving with left turns
        ...
        if (min_back_distance > 0.7): # if front is open, needs readjusting according to the new robot device
            if self.STRAIGHT: # drive straight mode
                ...
                if (min_distance > self.center_point+self.threshold):
                    ...
                    # wheel_msg.angle = -self.turn_angle*2*multiplier
            ...
            while (time.time()-now < 0.25):
                ...
                wheel_msg.throttle = -min(steer_throttle*1, 1.0)
                self.wheel_publisher.publish(wheel_msg)

            wheel_msg.angle = min(steer_angle*6.0,1.0)*multiplier
            wheel_msg.throttle = min(steer_throttle*1.2, 1.0)
        
        self.get_logger().info('Closest Distance(m) [B,R,L]: "%s", "%s", "%s"\n' % (min_back_distance, min_right_distance, min_left_distance))
...
def battery_check(driver):
    ...
    elif (response.level <= 7 and response.level > 3):
...
def main(args=None):
    ...
    backward_driver = BackwardDriver()
    rclpy.spin(backward_driver)
    backward_driver.destroy_node()
    rclpy.shutdown()
